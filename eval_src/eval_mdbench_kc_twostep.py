...
def process_data(data, client, model, sample_num=1, output_file=None):
    ...
    'Your tasks: \n'
    'Step 1: Carefully read the Reasoning Trace and extract all factual statements, assumptions, or claims that can be verified with external knowledge or evidence.\n'
    'Step 2: Reformulate each extracted item as a clear, self-contained knowledge question that could be checked for truthfulness (e.g., "Is X true?", "Did event Y happen in year Z?").\n' 
    'Step 3: Return all extracted questions in the following fixed format, without any additional text, commentary, or explanation.\n'
    'Format: <output> \n [Question 1] \n [Question 2] \n [Question 3] \n ... \n </output>')

    try:
        question_query = question_template.format(problem=text, think=first_think)
        questions, _, _ = call_model_no_think(question_query, client, model)
        match = re.search(r"<output>(.*?)</output>", questions)
        if match:
            questions = match.group(1).strip()  # Remove surrounding whitespace
    except Exception as e:
        print("[Exception type]:", type(e).__name__)
        traceback.print_exc()
        questions = None

    answer_template = ('Factual knowledge is information that aligns with objective reality and can be verified through '
        'evidence or observation, such as scientific facts or historical events.'
        'Please provide factual knowledge for the below question set: <Questions> {questions} <Questions>'
        'You should return a dictionary in JSON format; for each element in the dictionary, the key is each question in <Questions>,'
        'and the value is the factual knowledge of each question in <Questions>. '
        'Your answer format should strictly be in the following steps: <output> factual knowledge of questions </output>')
    try:
        answer_query = answer_template.format(questions=questions)
        question_answer, _, _ = call_model_no_think(answer_query, client, model)
        match = re.search(r"<output>(.*?)</output>", question_answer)
        if match:
            question_answer = match.group(1).strip()  # Remove surrounding whitespace
    except Exception as e:
        print("[Exception type]:", type(e).__name__)
        traceback.print_exc()
    ...
    detec_template = (
        'Given the following message, its step by step think process, and the related knowledge elements. \n'
        'Please predict its veracity. '
        'If it is more likely to be a real message, return \\boxed{{real}}; otherwise, return \\boxed{{fake}}. \n'
        'Message: {problem}, Think process: {think}, Knowledge elements: {knowledge}')

    try:
        query = detec_template.format(problem=text, think=first_think, knowledge=question_answer)
    ...
    data['question_answers'] = question_answer
    data['answer'] = answer
