...
def get_processed_ids(output_file):
    ...
    for existing_file in existing_files:
        if existing_file.endswith('jsonl'):
            candidate_files.append(existing_file.split('.')[0])
...
def process_data(data, client, model, sample_num=1, output_file=None):
    ...
    try:
        ...
        answer, think, output_token_length, input_token_length = call_model(query, client, model, system)
        data['answer'] = answer
        data['think'] = think
        ...
        # traceback.print_exc()  # standard error stack info (kept commented)

    return data


# Multiprocess task handler
...
if __name__ == "__main__":
    ...
    # Avoid embedding local absolute model paths; allow environment override.
    parser.add_argument('--model', type=str, default='', help="model path or id; if empty, use MODEL_PATH env var")
    ...
    for result in results:
        ...
        if answer is not None:
            # Here we treat the ground truth label "real" as positive (1) and others as negative (0).
            y_true.append(1 if label == 'real' else 0)
            # Prediction rule: treat answers containing 'real' as the positive class.
