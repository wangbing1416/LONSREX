...
# Check processed ID set for resuming interrupted jobs
...
def process_data(data, client, model, sample_num=1):
    text = data['text']
    desc_template = ('Q: Given the following message, predict its veracity. If it is more likely to be a real message, '
                     'return 1; otherwise, return 0. Please refrain from providing ambiguous assessments such as undetermined:'
                     ' \n {problem} A: Let’s think from the perspective of textual description. ')
    comm_template = ('Q: Given the following message, predict its veracity. If it is more likely to be a real message, '
                     'return 1; otherwise, return 0. Please refrain from providing ambiguous assessments such as undetermined:'
                     ' \n {problem} A: Let’s think from the perspective of commonsense. ')
    try:
        desc_query = desc_template.format(problem=text)
        desc_answer, _, _ = call_model(desc_query, client, model)
    except:
        desc_answer = None

    try:
        comm_query = comm_template.format(problem=text)
        comm_answer, _, _ = call_model(comm_query, client, model)
    except:
        comm_answer = None

    template = (
        'Here are two additional pieces of reference information to assist in evaluating the message\'s veracity: \n\n'
        'Text description: {desc_answer} \n\n Commonsense description: {comm_answer} \n\n'
    ...
    try:
        query = template.format(desc_answer=desc_answer, comm_answer=comm_answer, problem=text)
        answer, output_token_length, input_token_length = call_model(query, client, model)
        data['description'] = desc_answer
        data['commonsense'] = comm_answer
        data['answer'] = answer
